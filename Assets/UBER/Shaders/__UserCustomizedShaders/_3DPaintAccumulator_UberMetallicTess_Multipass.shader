Shader "Custom/Paint3DAccumulator_UberMetallicTess_Multipass"
{
	Properties
	{
       ...
       _TessEdgeLengthLimit ("Tessellation edge limit", Range(2,50)) = 5
		...
		// the names below need to be updated to the _P3DA versions. Check Assets\UBER\Shaders\__UserCustomizedShaders\UBER_StandardCore_3DPaintAccumulator.cginc
		// or MaterialInspector_P3DA.cs
		/*
	...
	CGINCLUDE
		...
		#pragma multi_compile __ _RUNNING_IN_PLAY_MODE_
		#pragma multi_compile __ _3D_PAINT_ACCUMULATOR_
		// 1
		#pragma multi_compile __ _CUSTOM_VERTEX_DISPLACEMENT_
		// 0
		#pragma multi_compile __ UNITY_REQUIRE_FRAG_WORLDPOS
		// 1

		#pragma multi_compile __ _GPU_COLOR_PICKER_
		#pragma multi_compile __ _BLEND_2_TEXTURES_WITH_PAINT_
		#pragma multi_compile __ _ERASE_MESH_WITH_PAINT_
		#pragma multi_compile __ _DISPLAY_BRUSH_VOLUME_
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "/UBER_StandardCore_Paint3DAccumulator.cginc"
			ENDCG
		}		
		*/
		...
		/* _Paint3DAccumulator_ */
		// ------------------------------------------------------------------
		//  _Paint3DAccumulator_ pass 0, backface only, paint accumulation
		Pass
		{
			Name "_Paint3DAccumulator_ P0 0" 
			Tags { "LightMode" = "Always" } //always means always rendered, no lighting: https://docs.unity3d.com/Manual/SL-PassTags.html
			...
			// replace with simple fragment, the Paint3DAccumulator-only fragment	
			//#pragma fragment fragForwardBase
			#pragma fragment fragPaint3DAccumulator
			...
			#pragma hull hs_surf
			// tessellation domain shader - this is where the v2f happens
			// replace with your own copy which includes 
			//#pragma domain ds_surf	
			#pragma domain ds_surfPaint3DAccumulator	
			...
			#include "Tessellation.cginc"
			#include "/UBER_StandardCore_3DPaintAccumulator.cginc"
			ENDCG
		}

		/* _Paint3DAccumulator_ */
		// ------------------------------------------------------------------
		//  _Paint3DAccumulator_ pass 0 1, frontface only, paint accumulation
		Pass
		{
			Name "_Paint3DAccumulator_ P0 1" 
			Tags { "LightMode" = "Always" } //always means always rendered, no lighting: https://docs.unity3d.com/Manual/SL-PassTags.html
		...
		/* _Paint3DAccumulator_ */
		// ------------------------------------------------------------------
		//  _Paint3DAccumulator_ pass 1 0, backface + base forward pass
		// ------------------------------------------------------------------
		...
		Pass
		{
			...
			ZWrite On// [_ZWrite]
			ZTest LEqual
			Cull Front /* _Paint3DAccumulator_ */
			//ZTest Always
		...
		/* _Paint3DAccumulator_ */
		// ------------------------------------------------------------------
		//  _Paint3DAccumulator_ pass 1 1, frontface + base forward pass
		// ------------------------------------------------------------------
		//  Base forward pass (directional light, emission, lightmaps, ...)
		Pass
		{
			...
			Blend SrcAlpha OneMinusSrcAlpha//[_SrcBlend] [_DstBlend]
			ZWrite On//[_ZWrite]//On// 
			ZTest LEqual
			Cull Back /* _Paint3DAccumulator_ */
		...
		Pass
		{
			...
			ZWrite Off
			ZTest LEqual
			Cull [_CullInFwdAdd]//Enum(Off,0,Front,1,Back,1) ///* _Paint3DAccumulator_ */
	...
	ENDCG
		...
		Pass
		{
			...
			#include "/UBER_StandardCore_3DPaintAccumulator.cginc"	ENDCG
		}
